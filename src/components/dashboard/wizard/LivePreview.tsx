    ...
    <div className="w-full h-[600px] bg-[#0E0E15] rounded-[24px] border border-[rgba(255,255,255,0.05)] shadow-2xl relative overflow-hidden flex flex-col items-center justify-center p-6">
      ...
      {!fabricPreview && !customerPreview && (
        <div className="text-center">
          ...
          <p className="text-[14px] text-[var(--text-muted)] font-[family-name:var(--font-body)]">Preview will appear here</p>
        </div>
      )}
          ...
          >
            ...
            {(fabricPreview && customerPreview) && (
              <motion.div
                ...
                className="absolute top-4 right-4 z-20 bg-[var(--accent-gold)]/10 border border-[var(--accent-gold)]/30 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-bold text-[var(--accent-gold)] uppercase tracking-wider"
              >
                Before AI
              </motion.div>
            )}
            ...
            <div className="flex-1 flex gap-2 relative">
              ...
              <AnimatePresence>
                {customerPreview && (
                  ...
                  >
                    <Image src={customerPreview} alt="Customer" fill className="object-cover" />
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <AnimatePresence>
                {fabricPreview && (
                  ...
                  >
                    <Image src={fabricPreview} alt="Fabric" fill className="object-cover" />
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            <AnimatePresence>
              {garmentType && (
                ...
                >
                  <span className="text-[11px] text-[var(--text-muted)] uppercase tracking-wider block mb-1">Rendering as</span>
                  <span className="text-[16px] font-[family-name:var(--font-display)] text-[var(--accent-gold)]">{garmentType}</span>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
